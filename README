UBER FOR REAL ESTATE (ANDROID)

We are making a marketplace like UBER which connects users & property brokers in Real Time. I have created an Android app which mimics the UBER interface to a large extent. It starts with a map which is used to pick up the user's location. The user is then prompted to enter 3 parameters of his choice (Number of Bedroom, Budget, Type of Transaction - Rent or purchase). On clicking the Hail button, all realtors in a 3-4 km radius are intimated. The realtor who accepts the users request gets connected to him instantly and can schedule the property visit from there on.
